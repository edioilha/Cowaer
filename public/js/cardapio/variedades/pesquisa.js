...
var dataTable = $('#tabela_dados').DataTable( {
		...
		// Ajax propriedades
		"ajax":{
			"url":"http://localhost/DuCheff/release/public/panel-control/variedades/pesquisa",
			"type":"POST"
		},
    ...
    $('#tabela_dados tbody').off("click",".view").on( 'click', '.view', function () {
        ...
        if ( row.child.isShown() ) {
           ...
           var codigo = parseInt(tr.children("td:eq(0)").text(),10);
		    ...
		    type: "GET",
		    url : "/ducheff/release/public/panel-control/variedades/editar",
		    data : {codigo:codigo},
		    dataType: 'json'
		    }).done(function(res){
...
$('#edicao').bootstrapValidator({
	...
	fields: {
		nome: {
			validators: {
				notEmpty: {
					message: pt_br.msg_erro_variedade
				}
			}
		}
...
$(document).off("click",".del").on("click",".del",function(){
	...
	$.ajax({
		type: "GET",
	    url: "/ducheff/release/public/panel-control/variedades/deletar",
	    data: {id:id}
    }).done(function(res){
...
function format(f){
	...
	string = '';
	string += "<b>"+pt_br.format_field_variedade+"</b>"+' '+f.nome+'<br>';
