...
------------------------------------------------------------------*/
var actions_buttons = '<a data-toggle="modal" data-target="#edit" class="editar"><i class="fa fa-edit" data-toggle="tooltip" data-placement="top" title="'+pt_br.tooltip_editar+'"></i></a> ';
...
var dataTable = $('#tabela_dados').DataTable( {
		...
		// Ajax propriedades
		"ajax":{
			"url":"http://localhost/DuCheff/release/public/panel-control/bebidas/pesquisa",
			"type":"POST"
		},
    ...
    $('#tabela_dados tbody').off("click",".view").on( 'click', '.view', function () {
        ...
        if ( row.child.isShown() ) {
           ...
           var codigo = parseInt(tr.children("td:eq(0)").text(),10);
		    ...
		    type: "GET",
		    url : "/ducheff/release/public/panel-control/bebidas/editar",
		    data : {codigo:codigo},
		    dataType: 'json'
		    }).done(function(res){
...
$(document).off("click",".editar").on("click",".editar",function(){
    ...
    dataType: 'json'
    	...
    	$("#titulo_modal").append("<b style='color:#d15e5e;'>"+res[0].nome.toUpperCase()+"</b>");
...
$(document).off("click",".del").on("click",".del",function(){
	...
	$.ajax({
		type: "GET",
	    url: "/ducheff/release/public/panel-control/bebidas/deletar",
	    data: {id:id}
    }).done(function(res){
    	...
    	{
    		alertErro(pt_br.msg_erro_relacao);
    	}
